...
def dedupe_characters(con: duckdb.DuckDBPyConnection) -> int:
    ...
    gives you a deterministic single row per id — the same dedup pattern as
    Week 2, defending against a different cause.
    """
    con.execute(
        """
...
def episode_appearances(con: duckdb.DuckDBPyConnection) -> int:
    ...
    DO THIS ONE IN POLARS, on purpose. ``episode`` is a list column; exploding it
