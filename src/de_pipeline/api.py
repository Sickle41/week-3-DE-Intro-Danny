...
response into S3 (the "land raw" principle), and the rest of the pipeline
...
def build_client(
    *,
    ...
    - Auth: if (and only if) a token is set, add
      ``Authorization: Bearer <token>`` — the pattern essentially every real API
      expects. Rick and Morty ignores it; wiring it is the point, and the tests
...
def fetch_page(page: int = 1, *, client: httpx.Client | None = None) -> dict:
    ...
    4s, ... capped). Tip: a custom ``wait`` callable receives the retry state, so
    ...
    def _fetch() -> dict:
        ...
        try:
            response = active_client.get("/character", params={"page": page})
            if response.status_code == 429:
                retry_after = response.headers.get("Retry-After")
                raise RateLimitError(float(retry_after) if retry_after is not None else None)
